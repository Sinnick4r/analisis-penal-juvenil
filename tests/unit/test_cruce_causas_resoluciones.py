...
class TestCalcularMetricasPorIpp:
    def test_input_vacio(self) -> None:
        vacio = pd.DataFrame(columns=[
            "ipp_canonico", "fecha_resolucion", "categoria_resolucion",
        ])
    ...
    def test_categorias_concatenadas_ordenadas(self, df_resoluciones_para_cruce_sint) -> None:
        ...
        assert ipp_3["categorias_resolucion"] == "cierre de proceso | competencia | medida de coercion"
...
class TestCruzar:
    ...
    def test_preserva_cantidad_de_filas(
        self, causas_con_canonico, df_resoluciones_para_cruce_sint,
    ) -> None:
...
class TestReporteAuditoria:
    def test_audit_completo(
        self, df_causas_para_cruce_sint, df_resoluciones_para_cruce_sint,
    ) -> None:
...
class TestAceptacionArchivosReales:
    ...
    def test_overlap_alto(self) -> None:
        ...
        assert overlap_pct >= 0.70, (
            f"Overlap esperado >= 70%, observado: {overlap_pct * 100:.1f}%"
        )
