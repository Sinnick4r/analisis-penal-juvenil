...
class TestNormalizarDelitos:
    ...
    def test_delito_no_informado_para_vacio(self, df_causas_sinteticas, dict_delitos_local_sint):
        df = normalizar_delitos(df_causas_sinteticas, dict_delitos_local_sint)
        # Fila 8: "" → "delito_no_informado"
        assert df.iloc[8]["delito_estandar"] == "delito_no_informado"
        assert df.iloc[8]["delito_informado"] == "no"

    def test_marca_proceso_especial(self, df_causas_sinteticas, dict_delitos_local_sint):
        df = normalizar_delitos(df_causas_sinteticas, dict_delitos_local_sint)
        # Fila 7: "amparo" → es proceso especial
        assert df.iloc[7]["es_proceso_especial"]

    def test_flag_agravado_se_activa(self, df_causas_sinteticas, dict_delitos_local_sint):
        df = normalizar_delitos(df_causas_sinteticas, dict_delitos_local_sint)
    ...
    def test_sub_agravante_escalamiento(self, df_causas_sinteticas, dict_delitos_local_sint):
        df = normalizar_delitos(df_causas_sinteticas, dict_delitos_local_sint)
        # Fila 4: "hurto agravado por escalamiento"
        assert df.iloc[4]["agravado_flag"]
        assert df.iloc[4]["agravante_escalamiento"]

    def test_agravante_no_especificado(self, df_causas_sinteticas, dict_delitos_local_sint):
        df = normalizar_delitos(df_causas_sinteticas, dict_delitos_local_sint)
    ...
    def test_posible_delito_multiple_se_marca(self, df_causas_sinteticas, dict_delitos_local_sint):
        df = normalizar_delitos(df_causas_sinteticas, dict_delitos_local_sint)
        # Fila 5: "lesiones leves y amenazas" → múltiple
        assert df.iloc[5]["posible_delito_multiple"]

    def test_excepcion_no_marca_multiple(self, df_causas_sinteticas, dict_delitos_local_sint):
        df = normalizar_delitos(df_causas_sinteticas, dict_delitos_local_sint)
